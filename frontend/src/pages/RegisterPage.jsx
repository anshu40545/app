...
import { Eye, EyeOff, Mail, Lock, User, ArrowRight, Loader2, Check, X, Github } from 'lucide-react';
...
const RegisterPage = () => {
  ...
  const { register, initiateGithubLogin, checkGithubEnabled } = useAuth();
  ...
  useEffect(() => {
    const checkGithub = async () => {
      const enabled = await checkGithubEnabled();
  ...
  const handleGithubLogin = async () => {
    ...
    // If successful, page will redirect to GitHub
  };
          ...
          <div className="bg-white rounded-2xl shadow-lg border border-slate-200 p-8">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <Button
                type="button"
                variant="outline"
                className={`h-12 rounded-xl border-slate-200 hover:bg-slate-50 ${!githubEnabled ? 'opacity-50' : ''}`}
                onClick={handleGithubLogin}
                disabled={githubLoading || !githubEnabled}
              >
                {githubLoading ? (
                  <Loader2 className="w-5 h-5 mr-2 animate-spin" />
                ) : (
